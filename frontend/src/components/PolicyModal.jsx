...
function PrivacyContent() {
    return (
        <>
            ...
            <ul style={{ margin: '0 0 8px 16px', padding: 0 }}>
                <LI><strong>雲端資料服務（Google Sheets）：</strong>用於儲存公告、意見回饋、共享平台貼文及匿名使用分析事件。</LI>
            ...
            <H2>八、資料保留與刪除</H2>
            <P><strong>本機資料：</strong>可隨時於「設定 → 清除資料與登出」清除。<strong>伺服器端帳密快取：</strong>登出後立即清除；最長保留 24 小時，伺服器重啟後自動清空。<strong>意見回饋與共享平台內容：</strong>由管理員於後端雲端服務中管理，如需刪除請透過意見回饋功能聯繫。<strong>匿名分析事件：</strong>不含個人資訊，不提供個別刪除。</P>
    ...
    return (
        <>
            ...
            <ul style={{ margin: '0 0 8px 16px', padding: 0 }}>
                ...
                <LI>了解並接受本應用程式的帳密處理方式（詳見隱私權政策第三條）：帳密將暫時存放於伺服器記憶體最長 24 小時，以供後續資料請求使用</LI>
                <LI>若不希望帳密保留於伺服器，應在使用後立即於設定中選擇「清除資料與登出」</LI>
            ...
            <P>嘗試繞過上述限制（如使用自動化工具或模擬多個裝置）屬違規行為，可能導致封鎖或通報。</P>
            ...
            <ul style={{ margin: '0 0 8px 16px', padding: 0 }}>
                ...
                <LI>伺服器記憶體中的帳密快取不因系統異常、入侵或意外事件而洩漏（本應用程式已採取合理安全措施，但無法提供絕對保證）</LI>
            </ul>
